...
<template>
  ...
  >
    <div class="pointer-events-none absolute inset-0 -z-10 whimsy:hidden" aria-hidden="true">
      ...
      <div class="absolute -bottom-16 -right-24 h-[340px] w-[340px] rounded-full bg-sky/15 blur-[130px]" />
    </div>
    ...
    <div v-reveal class="mx-auto flex w-full max-w-5xl flex-col gap-8 md:flex-row md:gap-12">
      <div class="flex flex-1 flex-col gap-4">
        ...
        <div class="flex flex-col gap-2.5 pt-2 font-mono text-sm font-bold">
          <div class="flex items-center gap-2">
            ...
            <a :href="`mailto:${CONTACT_EMAIL}`" class="text-xs text-paper transition-colors duration-300 hover:text-sky">{{
              CONTACT_EMAIL
            }}</a>
          </div>
        </div>
      ...
      <form
        class="flex flex-1 flex-col gap-4 rounded-xl border border-muted p-5 shadow-[0px_4px_16px_0px_rgba(2,22,125,0.02)] whimsy:border-[3px] whimsy:border-blue whimsy:bg-ink whimsy:ring-[3px] whimsy:ring-ink"
      ...
      >
        <div class="flex flex-col gap-1.5">
          ...
          <input
            ...
            class="h-10 rounded-md border border-muted bg-paper px-3 text-[13px] text-ink placeholder:text-subtle"
          />
        </div>
        <div class="flex flex-col gap-1.5">
        ...
        <div class="flex flex-col gap-1.5">
          ...
          <textarea
            ...
            class="rounded-md border border-muted bg-paper px-3 py-2.5 text-[13px] text-ink placeholder:text-subtle"
          />
        </div>
        <button
          type="submit"
          class="btn-animated flex items-center justify-center gap-2 rounded-md bg-gradient-to-r from-blue via-indigo to-violet py-3 font-mono text-xs font-bold uppercase text-white hover:shadow-lg hover:shadow-indigo/35"
        >
          Send Message
    ...
    >
      ...
      <button
        type="button"
        class="font-mono font-bold text-cyan transition-colors duration-300 hover:text-violet"
        @click="scrollToTop"
      >
        BACK TO TOP ↑
      </button>
