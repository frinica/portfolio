  ...
  >
    ...
    <div v-reveal class="mx-auto flex w-full max-w-5xl flex-col gap-8 md:flex-row md:gap-12">
      <div class="flex flex-1 flex-col gap-4">
        ...
        <div class="flex flex-col gap-2.5 pt-2 font-mono text-sm font-bold">
          <div class="flex items-center gap-2">
            ...
            <a :href="`mailto:${CONTACT_EMAIL}`" class="text-xs text-paper transition-colors duration-300 hover:text-teal">{{
              CONTACT_EMAIL
            }}</a>
          </div>
        </div>
      ...
      >
        ...
        <button
          type="submit"
          class="btn-animated flex items-center justify-center gap-2 rounded-md bg-gradient-to-r from-teal via-violet to-cyan py-3 font-mono text-xs font-bold uppercase text-white hover:shadow-lg hover:shadow-cyan/35"
        >
          Send Message
