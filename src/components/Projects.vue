...
const cardAccents = [
  { ring: "from-teal to-sky", shadow: "hover:shadow-teal/25" },
  { ring: "from-blue to-violet", shadow: "hover:shadow-violet/25" },
]
</script>
    ...
    <div v-reveal class="mx-auto flex w-full max-w-5xl flex-col gap-6">
      ...
      <div class="grid w-full grid-cols-1 gap-5 md:grid-cols-2">
        <div v-for="(project, i) in projects" :key="project.slug" v-reveal="i * 120">
          <div
            class="group h-full rounded-xl bg-gradient-to-br p-[1.5px] shadow-[0px_4px_16px_0px_rgba(2,22,125,0.02)] transition-all duration-300 hover:-translate-y-1 hover:shadow-lg whimsy:p-2 whimsy:ring-[3px] whimsy:ring-ink"
          ...
          >
            <article class="flex h-full flex-col overflow-hidden rounded-[10px] bg-ink">
              ...
              <div class="flex flex-col gap-3 px-4 pb-5 pt-4">
                ...
                <button
                  type="button"
                  class="group/link flex items-center gap-1.5 pt-1 font-mono text-xs font-bold text-paper hover:text-sky"
        ...
        <div v-reveal="240" class="md:col-span-2">
          <article
            class="flex flex-col justify-center gap-3 rounded-xl border border-dashed border-haze p-6 text-center transition-colors duration-300 hover:border-violet whimsy:border-[3px] whimsy:border-solid whimsy:border-violet whimsy:bg-ink whimsy:ring-[3px] whimsy:ring-ink"
          >
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              class="group/link mx-auto flex items-center gap-1.5 pt-1 font-mono text-xs font-bold text-paper hover:text-sky"
            >
              VIEW GITHUB PROFILE
              <ArrowIcon :size="10" color="currentColor" class="transition-transform duration-300 group-hover/link:translate-x-1" />
            </a>
